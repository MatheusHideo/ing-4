...
def execute():
    ...
    if not df.empty:
        ...
        csv_file_path = os.path.join(output_directory, 'bancos.csv')


        # Salvar o DataFrame em um arquivo JSON
        df_csv = df.fillna(pd.NA)
        df_csv.to_csv(csv_file_path, index=False, sep="|")
        df_json = df.fillna('null')
        df_json.to_json(json_file_path, orient='records', lines=True, force_ascii=False)
