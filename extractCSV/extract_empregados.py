...
def execute():
    ...
    if not empregados_df.empty:
        ...
        empregados_df_csv = empregados_df
        empregados_df_csv.fillna(pd.NA, inplace=True)
        empregados_df.fillna('null')
        ...
        empregados_df_csv = empregados_df_csv.replace('null', pd.NA)
        empregados_df_csv.to_csv(f'{bronze_path}/empregados.csv', sep="|", index=False)
    else:
